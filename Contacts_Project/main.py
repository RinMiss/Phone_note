...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        btn_edit_dialog = tk.Button(toolbar, bg='#d7d8e0', border=0, image=self.update_img, command=self.open_update_dialog)
    ...
    def update_record(self, name, tel, email):
        self.db.conn.execute(''' UPDATE db SET name=?, tel=?, email=?, WHERE ID = ?''', 
                             (name, tel, email, (self.tree.selection()[0], '#1')))
        self.db.conn.commit()
        self.view_records()
     # Метод удаления строк
    def delete_records(self):
        for celection_item in self.tree.selection():
            self.db.c.execute(''' DELETE FROM db WHERE id = ?''',
                               (self.tree.set(celection_item, '#1'),))
            self.db.conn.commit()
            self.view_records()
...
# Инициализируем виджеты для дочернего окна
    def init_child(self):
        ...
        self.button_ok.place(x=200, y=170)
        self.button_ok.bind('<Button-1>',lambda event:self.view.records(self.entry_name.get(),self.entry_email.get(),self.entry_tel.get()))
        

#Kласс дочернего окна для изменения данных        
class Update(Child):
    def __init__(self):
        super().__init__()
        self.init_edit()
